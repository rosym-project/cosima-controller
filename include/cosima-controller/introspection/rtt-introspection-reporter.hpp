...
private:
    ...
    std::vector<std::shared_ptr<RTT::InputPort<std::vector<cosima_msgs::CallTraceSample> > > > in_ctsamples_ports;
    std::vector<std::vector<cosima_msgs::CallTraceSample> > in_ctsamples_vars;
    // std::vector<RTT::FlowStatus> in_ctsamples_flows;

    std::vector<cosima_msgs::CallTraceSample> in_current_var;
    RTT::FlowStatus in_current_flow;

    std::vector<cosima_msgs::CallTraceSample> ctsamples_storage;
    uint storage_size;
