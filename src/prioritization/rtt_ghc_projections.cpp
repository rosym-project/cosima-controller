/* 
Author: Niels Dehio, Dennis Leroy Wigand <dwigand@techfak.uni-bielefeld.de>
...
#include "../../include/cosima-controller/prioritization/rtt_ghc_projections.hpp"
...
GHCProjections::GHCProjections() : Jaugmented_task_pos(0)
{
...
void GHCProjections::init(unsigned int numTasks, Eigen::VectorXd tasksize, unsigned int DOFsize)
{
    ...
    inertia_debug = Eigen::MatrixXd::Identity(DOFsize, DOFsize);
...
{
    this->inertia_debug = inertia;
    this->inertiaInv = inertia.inverse();
    this->L = chol.compute(this->inertiaInv).matrixL(); //lower triangular matrix
}

Eigen::MatrixXd GHCProjections::getInertiaMatrixInv()
{
    return this->inertiaInv;
}

Eigen::MatrixXd GHCProjections::getInertiaMatrix()
{
    return this->inertia_debug;
}
...
double GHCProjections::getAlphaLastEntry()
{
    return alphas(numTasks - 1, numTasks - 1);
}
...
// void GHCProjections::iterativelyBuildJaugmented(const Eigen::MatrixXd &J, const unsigned int taskNumber)
// {
//     unsigned int tsize = Tasksize[taskNumber];
//     Jaugmented.block(Jaugmented_task_pos, 0, tsize, DOFsize) = J;
//     Jaugmented_task_pos += tsize;
// }
