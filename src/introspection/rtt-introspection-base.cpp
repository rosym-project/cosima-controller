 ...
 *
 * ============================================================ */
#include "../../include/cosima-controller/introspection/rtt-introspection-base.hpp"
																	  ...
																	  auto_write_execution_information(false)
{
    ...
    executionTimes.reserve(5000);
	executionTimes.clear();
    executionTimes_latest = 0;

    this->provides("introspection")->addProperty("latestCallTraceSample", latestCallTraceSample);
}
...
bool RTTIntrospectionBase::configureHook()
{
    ...
    cts_start = cosima_msgs::CallTraceSample();
    cts_start.callName = "startHook()";
    cts_start.containerName = this->getName();
    cts_start.call_time = 0.0;
    cts_start.call_duration = 0.0;
    cts_start.call_type = CALL_UNIVERSAL;

	// cts_configure = cosima_msgs::CallTraceSample();
    // cts_configure.callName = "configureHook()";
    // cts_configure.containerName = this->getName();
    // cts_configure.call_time = 0.0;
    // cts_configure.call_duration = 0.0;
    // cts_configure.call_type = CALL_UNIVERSAL;

    cts_update = cosima_msgs::CallTraceSample();
    cts_update.callName = "updateHook()";
    cts_update.containerName = this->getName();
    cts_update.call_time = 0.0;
    cts_update.call_duration = 0.0;
    cts_update.call_type = CALL_UNIVERSAL;

    // cts_stop = cosima_msgs::CallTraceSample();
    // cts_stop.callName = "stopHook()";
    // cts_stop.containerName = this->getName();
    // cts_stop.call_time = 0.0;
    // cts_stop.call_duration = 0.0;
    // cts_stop.call_type = CALL_UNIVERSAL;

    // cts_cleanup = cosima_msgs::CallTraceSample();
    // cts_cleanup.callName = "cleanupHook()";
    // cts_cleanup.containerName = this->getName();
    // cts_cleanup.call_time = 0.0;
    // cts_cleanup.call_duration = 0.0;
    // cts_cleanup.call_type = CALL_UNIVERSAL;

    cts_port = cosima_msgs::CallTraceSample();
    cts_port.callName = "port_access######################################";
    cts_port.containerName = this->getName();
    cts_port.call_time = 0.0;
    cts_port.call_duration = 0.0;
    cts_port.call_type = CALL_UNIVERSAL;

    // 
    latestCallTraceSample = cosima_msgs::CallTraceSample();
    latestCallTraceSample.callName = "port_access######################################";
    latestCallTraceSample.containerName = this->getName();
    latestCallTraceSample.call_time = 0.0;
    latestCallTraceSample.call_duration = 0.0;
    latestCallTraceSample.call_type = CALL_UNIVERSAL;
...
void RTTIntrospectionBase::updateHook()
{
	...
	{
		cts_update.call_time = overhead_start;
		cts_update.call_type = CALL_START_WITH_DURATION;
		...
		// Send once before an overflow will happen AND once per (ms) if required.
		// Otherwise it may happen that the buffer will never get full before the application is shut down
		// and we do not get any data.
		// We also cannot wait until a component is stopped to send the collected data,
		// because we do not know at that time whether or not the collector component is stopped or still running to receive the data samples.
		if ((call_trace_storage.size() >= call_trace_storage_size) || (send_at_least_once_per_Xms > 0 && ((overhead_start - last_send) * 1E-6 >= send_at_least_once_per_Xms)))
		{
			// done = true;
			...
			// Take the time from the actual sending point in time, or the  point in time when the updateHook() was called (overhead_start).
			last_send = time_service->getNSecs();
		}
        ...
        latestCallTraceSample = cts_update;

		uint_least64_t overhead_end = time_service->getNSecs();
        executionTimes_latest = overhead_end - overhead_start;
		if (executionTimes.size() < executionTimes.capacity())
		{
			executionTimes.push_back(executionTimes_latest);
		}
...
bool RTTIntrospectionBase::startHook()
{
	...
	if (useCallTraceIntrospection)
	{
		...
		cts_start.call_type = CALL_START_WITH_DURATION;
		...
		// end intro
		// cts_start.call_type = CALL_END;
		cts_start.call_duration = time_service->getNSecs();

		// Initialize the last_send with the actual time, so that we do not send at first sight (with time_service->getNSecs())
		last_send = cts_start.call_duration;
...
void RTTIntrospectionBase::stopHook()
{
	if (useCallTraceIntrospection)
	{
		...
		// cts_stop.call_type = CALL_START_WITH_DURATION;
...
void RTTIntrospectionBase::processCTS(cosima_msgs::CallTraceSample &cts)
{
	if (call_trace_storage.size() >= call_trace_storage_size)
	{
		// done = true;
...
cosima_msgs::CallTraceSample RTTIntrospectionBase::getLatestCallTraceSample()
{
	return this->latestCallTraceSample;
}
