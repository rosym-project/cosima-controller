 ...
 *
 * ============================================================ */
#include "../../include/cosima-controller/introspection/rtt-introspection-reporter.hpp"
...
bool IntrospectionReporter::configureHook()
{
    for (std::string peerName : this->getPeerList())
    {
        ...
        std::shared_ptr<RTT::InputPort<std::vector<cosima_msgs::CallTraceSample>>> ipi(new RTT::InputPort<std::vector<cosima_msgs::CallTraceSample>>("in_" + peerName + "_port"));
    ...
    // clean all ports
    for (std::shared_ptr<RTT::InputPort<std::vector<cosima_msgs::CallTraceSample>>> port : in_ctsamples_ports)
    {
        port->clear();
    }
    return true;
...
{
    log(Debug) << "Logger updateHook " << endlog();
    if (!this->isConfigured() || ctsamples_storage.size() == ctsamples_storage.capacity())
    {
        log(Error) << "Logger abort due to initial if (!this->isConfigured() || ctsamples_storage.size() == ctsamples_storage.capacity())" << endlog();
        return;
    }
    ...
    for (std::shared_ptr<RTT::InputPort<std::vector<cosima_msgs::CallTraceSample>>> port : in_ctsamples_ports)
    {
        ...
        log(Debug) << "for each port " << port->getName() << " read: " << in_current_flow << endlog();
...
void IntrospectionReporter::stopHook()
{
    ...
    bool first = true;
    for (cosima_msgs::CallTraceSample cts : ctsamples_storage)
    {
        if (first)
        {
            first = false;
            myfile
            // << cts;
                << "{\"call_name\":\"" << cts.callName << "\""
				   << ",\"container_name\":\"" << cts.containerName << "\""
				   << ",\"call_time\":\"" << cts.call_time << "\""
				   << ",\"call_duration\":\"" << cts.call_duration << "\""
				   << ",\"call_type\":\"" << (int)cts.call_type << "\"}";
        }
        else
        {
            myfile << ",\n"
